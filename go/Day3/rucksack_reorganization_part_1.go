...
func prioritizeItems(rucksacks string) int {
	...
	sum := 0
	for _, rucsack := range splittedRucksack {
		lRucksack, rRucksack := rucsack[:len(rucsack)/2], rucsack[len(rucsack)/2:]

		for _, item := range lRucksack {
			if strings.Contains(rRucksack, string(item)) {
